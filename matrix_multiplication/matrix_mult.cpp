...
float *matrix_ref=(float *) malloc(sizeof(float)*N*N);
...
for(unsigned i = 0; i < N; ++i) {
      ...
      for(unsigned n = 0; n < N; n++){
        tmp+=input_a[i*N+n]*input_b[n*N+j];
      }
      matrix_ref[i*N+j]=tmp;
      }
}
    ...
    time (&start);
     ...
     size_t num2[num];
     clGetDeviceInfo(device,CL_DEVICE_MAX_WORK_ITEM_SIZES,sizeof(size_t)*num,&num2,NULL);
...
for(unsigned j = 0; j < N; ++j) {
      for(unsigned i = 0; i < N; ++i){
        if (fabsf(matrix_ref[j*N+i] -output[j*N+i]) > 1.0e-5f){
          printf ("result %f  ref %f",output[j*N+i],matrix_ref[j*N+i]);
          f=1;
       }
      }
}
if (f==0)cout<<"verification finished, no error"<<endl<<endl;
